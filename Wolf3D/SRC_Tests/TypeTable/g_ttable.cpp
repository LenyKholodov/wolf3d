...
struct  ObjectInfoTag: public GameObjectTag
{
  ...
  ObjectInfoTag   (GameObjects __type,ID __id = ID(-1,-1),ID cb = ID(-1,-1))
        : id (__id), type(__type), callBack (cb)
         {
           tag  = gameTags [__type];
         }
};
...
void    TypeTable::Load (BinFile* file)
{
  ...
  file->read (mTable,sizeof(TypeTableNode)*MAX_TABLES*MAX_SLOTS);
  file->read (mSize,sizeof(int16)*MAX_TABLES);
  file->read (mNext,sizeof(int16)*MAX_TABLES);
  file->read (mTypes,sizeof(int16)*MAX_TYPES);

  for (int t=0;t<MAX_TYPES;t++)
      for (int slot=0;slot<MAX_SLOTS;slot++)
          mTable[t][slot].data = NO_OBJECT;
...
void    TypeTable::Save (BinFile* file)
{
  ...
  file->write (mTable,sizeof(TypeTableNode)*MAX_TABLES*MAX_SLOTS);
  file->write (mSize,sizeof(int16)*MAX_TABLES);
  ...
  for (int t = 0;t<MAX_TYPES;t++)
      for (int slot = 0;slot<MAX_SLOTS;slot++)
      {
          ...
          ObjectInfoTag gameTag (current->type(),ID(t,slot),current->callBack);
