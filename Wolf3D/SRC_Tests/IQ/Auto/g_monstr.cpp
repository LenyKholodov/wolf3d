...
void    Monstr::think         ()
{
  ...
  IQClass::Actives active = (IQClass::Actives)-1;

  for (int i=0;i!=active;i=active)
      if (table [i])
      {
         active = table[i]->analyse (param);
         dout<<"This "<<i<<" Next "<<active<<endl;
      }

  curMethod           = table[active];
  param.previos       = (IQClass::Actives)active;

  dout<<"Find method "<<active<<endl;
}
