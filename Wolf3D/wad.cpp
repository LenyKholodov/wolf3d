...
WadFile :: WadFile ( const char * name )
{
        ...
        directory = new DirEntry [entries = hdr.dirEntries];
...
tPic   * WadFile :: loadPic ( const char * picName )
{
...
tTexture * WadFile :: loadTexture ( const char * texName )
{
        ...
        for ( int i = 0; i < texture1 [0]; i++ )
        {
                ...
                {
                        tTexture * tex = new tTexture;    // init tTexture
...
void    WadFile :: applyPatch ( tTexture * tex, TexturePatch& patch )
{
        tPic   * pic = loadPic ( pnames + 2 + patch.pnamesNo * 8 );
...
tTexture * WadFile :: loadFloorTexture ( const char * name )
{
...
int           WadFile::findEntries  ( const char* first, char**  table)
{
  int   num = 0;
  int   len = strlen (first);

  for (int i=0;i<entries;i++)
      if (!memcmp (first,directory[i].name,len))
      {
        table [num] = new char [8];
        memcpy (table[num++],directory[i].name,8);
      }
