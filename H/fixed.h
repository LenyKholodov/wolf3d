...
public:
        ...
        Fixed16 operator+(Fixed16 x) const { Fixed16 a=*this; a+=x; return a; };
        ...
        operator=(const Fixed8 x)    { data = x.data<<8;  }
        operator=(const Fixed16 x)   { data = x.data;  }
        operator=(const int x)       { data = x<<16; }
        operator=(const double& x)   { data=x*65536.0;  }
