...
void   _loadds far GlobalHandler(int,int,int,int,Mouse::Buttons) { }
void   _loadds far GlobalPress(int,int,int,int,int,Mouse::Buttons) {}


        //Следующие две процедуры получают управление
        //при перемещении или нажатии (0xC)
#pragma off (check_stack)
#pragma aux _move_handler parm        [ECX] [EDX] [ESI] [EDI] [EBX] //bl
#pragma aux _press_handler parm       [EAX] [ECX] [EDX] [ESI] [EDI] [EBX]//bl

void   _loadds far _move_handler   (int x,int y,int _x,int _y,Mouse::Buttons but)
{
  MouseMove::user_handler(x,y,_x,_y,but);
}

void   _loadds far _press_handler   (int ev,int x,int y,int _x,int _y,Mouse::Buttons but)
{
  MousePress::user_handler(ev,x,y,_x,_y,but);
}
...
static  void  (far* MouseMove::user_handler)(int,int,int,int,Mouse::Buttons)  = GlobalHandler;
static  void  (far* MousePress::user_handler)(int,int,int,int,int,Mouse::Buttons) = GlobalPress;
...
MouseMove::MouseMove(void (far* user)(int,int,int,int,Mouse::Buttons))
...
MousePress::MousePress(Mouse::Event e,void (far* user)(int,int,int,int,int,Mouse::Buttons))
...
MouseMessage::MouseMessage()
{
  if (count++ == 0)
  {
    handler   = new MousePress ((Mouse::Event)0xFF,
              (void (far*)(int,int,int,int,int,Mouse::Buttons))_handler);
