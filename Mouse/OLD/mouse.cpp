...
#pragma aux _move_handler parm        [ESI] [EDI] [EBX]         //dx cx
#pragma aux _press_handler parm       [EAX] [ESI] [EDI] [EBX]   //dx cx
...
void    MouseScreen::SetCursor(tSprite* spr)
{
  ...
  store  = (tImage*)malloc(spr->width*spr->height*owner->bpp()
                           +sizeof(tImage)-1);
//  store->width  = spr->width;
//  store->height = spr->height;
...
{
  owner->GetImage  (oldX=x,oldY=y,width,height,store);
...
{
  if (!visible)
     _showmouse(Mouse::GetScanX()>>1,Mouse::GetScanY()>>1);
  visible = TRUE;
}
